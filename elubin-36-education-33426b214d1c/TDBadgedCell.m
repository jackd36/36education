...
{		
	...
	CGRect bounds = CGRectMake(0 , 0, numberSize.width + 12 , numberSize.height+2);
...
- (void) layoutSubviews
{
	...
	if(self.badgeString)
	{
		...
		if(self.textLabel.right >= badgeframe.origin.x){
            self.textLabel.width = badgeframe.origin.x-10.0f-self.textLabel.left;
        }
        if(self.detailTextLabel.right >= badgeframe.origin.x){
            self.detailTextLabel.width = badgeframe.origin.x-10.0f-self.detailTextLabel.left;
        }
